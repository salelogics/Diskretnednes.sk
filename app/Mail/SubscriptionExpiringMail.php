<?php

namespace App\Mail;

use Illuminate\Mail\Mailable;
use Illuminate\Mail\Mailables\Content;
use Illuminate\Mail\Mailables\Envelope;
use Illuminate\Queue\SerializesModels;
use App\Models\Ad;
use App\Models\EmailTemplate;

class SubscriptionExpiringMail extends Mailable
{
    use SerializesModels;

    public $ad;
    public $daysLeft;
    protected $emailTemplate;

    /**
     * Create a new message instance.
     */
    public function __construct(Ad $ad, int $daysLeft = 3)
    {
        $this->ad = $ad;
        $this->daysLeft = $daysLeft;
        
        // Načítame EmailTemplate z databázy
        $this->emailTemplate = EmailTemplate::getByKey('subscription_expiring');
        
        // Ak template neexistuje, vytvoríme fallback
        if (!$this->emailTemplate) {
            \Log::warning('EmailTemplate "subscription_expiring" not found, using fallback');
        }
    }

    /**
     * Get the message envelope.
     */
    public function envelope(): Envelope
    {
        $subject = 'Predplatné čoskoro vyprší - DiskretneDnes.sk';
        
        if ($this->emailTemplate) {
            $data = $this->getTemplateData();
            $subject = $this->emailTemplate->renderSubject($data);
        }

        return new Envelope(
            subject: $subject,
        );
    }

    /**
     * Get the message content definition.
     */
    public function content(): Content
    {
        if ($this->emailTemplate) {
            // Používame EmailTemplate z administrácie
            $data = $this->getTemplateData();
            $htmlContent = $this->emailTemplate->renderContent($data);
            
            return new Content(
                html: $htmlContent,
            );
        }
        
        // Fallback na pôvodný view ak template neexistuje
        return new Content(
            view: 'emails.subscription-expiring',
            with: [
                'ad' => $this->ad,
                'user' => $this->ad->user,
                'daysLeft' => $this->daysLeft,
            ],
        );
    }

    /**
     * Pripraví dáta pre template
     */
    private function getTemplateData(): array
    {
        return [
            'user_name' => $this->ad->user->name ?? 'Používateľ',
            'ad_id' => $this->ad->id,
            'ad_nickname' => $this->ad->nickname ?? 'Neznámy',
            'expiry_date' => $this->ad->expires_at ? $this->ad->expires_at->format('d.m.Y') : 'Neznámy',
            'days_left' => $this->daysLeft,
        ];
    }

    /**
     * Get the attachments for the message.
     *
     * @return array<int, \Illuminate\Mail\Mailables\Attachment>
     */
    public function attachments(): array
    {
        return [];
    }
}
